import { pgTable, index, serial, text, varchar, smallint, integer, vector, timestamp, unique, boolean, foreignKey, bigint, jsonb, pgEnum } from "drizzle-orm/pg-core"
...
export const catalog = pgTable("catalog", {
	...
	index("catalog_embedding_hnsw_idx").using("hnsw", table.embedding.asc().nullsLast().op("vector_cosine_ops")).with({m: "8",ef_construction: "64"}),
