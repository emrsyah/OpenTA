...
export async function POST(req: NextRequest) {
  try {
    ...
    // (messages array only contains the current conversation's messages)
    const isFirstMessage = messages.filter((m) => m.role === "user").length <= 1;

    // Check if conversation exists to determine first-message status more accurately
    // If conversationId is provided and there are existing messages, it's not the first
    const effectiveIsFirstMessage = conversationId
      ? messages.filter((m) => m.role === "user").length <= 1
      : true;

    // Stream the agent response directly — no proxy to Python backend
